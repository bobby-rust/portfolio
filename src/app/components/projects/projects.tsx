    ...
    const projects: Project[] = [
        {
            ...
            extendedDescription: `This artifact is QueueSocial, a social media post scheduler. I first began creating this project about 8 months ago, had it almost complete, but ran into so much technical debt and made so many wrong decisions about technologies and architecture that I decided it was best to just completely rewrite it. This way, I can start fresh and keep it simple using what I learned from my previous attempt. I selected this item because I feel that it is a good project to demonstrate my skills in building real-world applications. 

 The project utilizes the Facebook API, which demonstrates that I can work with large production APIs. The front end shows that I’m capable of building clean user interfaces, and the back end shows that I understand how data flows through an application. It also demonstrates modern security best practices by utilizing JWTs for authentication. In this milestone, I improved the artifact by implementing the Create Post page on the front end, which has a form for the user to submit text and upload an image for their post. I created an S3 bucket to store the image where I can then use the URL to upload the image to Facebook. I also created an endpoint to create a post, and two separate functions to handle posts with and without images. This is ready to be used, but the application still needs to obtain access tokens and save data to the MongoDB database for it to be working fully, which will be completed in another milestone, and the application will be fully able to schedule Facebook posts for users.  
...
I faced many challenges when updating this artifact. It was my first time using any AWS services, so getting the S3 bucket and AWS SDK set up with my credentials took a while. Then when I was finally able to upload images to the bucket via the SDK, the images were not accessible and were private, so I had to debug that and ended up needing to make them publicly accessible. However, after working with AWS S3, I am much more confident in my abilities to utilize AWS services, and truthfully, I thought it would be more complicated than it was. AWS does a very good job in creating their documentation which was extremely helpful. Other than learning new things, I got to hone my skills and gain more experience in areas I am already familiar with in full-stack development. `,
        ...
        },
        {
            ...
            extendedDescription: `This artifact was created for my data structures and algorithms course that I took about 2 years ago. It is a university course scheduler that uses a CSV file containing courses and their prerequisites and creates a schedule such that no course is taken before all its prerequisites have been met. It initially used a binary search tree where the child nodes are dependent on its parent, but this came with the constraint that there can only be one course with no prerequisites, the root node of the tree. It also had many other issues. Consider the two children of the root node. These two children can each only have one prerequisite, and we can have no more than two courses with one prerequisite. In fact, the number of prerequisites each node can have is equivalent to its depth in the tree, and the number of courses that can have that number of prerequisites is constrained to . Since we said that the root node has no prerequisites, we could create a binary tree for each course with no prerequisites. But then consider the case that CS300 has prerequisites CS200 and MAT201. The binary trees would then need to be connected, which would form a graph.
	To address these concerns, I have chosen to implement a directed acyclic graph to create university course schedules for students. This way we can use topological sorting to find a valid schedule. I implemented Kahn’s algorithm, which is a topological sorting algorithm for digraphs.  This algorithm is useful because we know that if the algorithm has finished and there are still nodes in the graph, then there is a cycle in the graph, and we have an invalid input. This enhancement demonstrates a strong understanding of data structures and algorithms by applying theoretical concepts to solve real-world problems effectively and efficiently.
	For this enhancement, I have fulfilled the course outcome “Design and evaluate computing solutions that solve a given problem using algorithmic principles and computer science practices and standards appropriate to its solution, while managing the trade-offs involved in design choices (data structures and algorithms).” When fulfilling this outcome, I got more comfortable using my theoretical knowledge of graphs by implementing them in code for the first time. At first, I wasn’t sure how to represent the graph. I had only implemented trees up to this point, and I knew a graph was similar to a tree, so their implementation could be similar. I discovered that instead of hard coding a number of nodes that are connected to the node, I could use a vector of nodes to store the node’s dependencies. In this way, the dependencies of a node point to that node.`,
